...
Do not use `hermes config set mcp_servers.openrouter.tools.include '[...]'` for this list: `hermes config set` treats that JSON-looking value as a string on current Hermes versions, and the MCP filter will not behave as intended. If a non-interactive repair is necessary, use Hermes' own config loader/writer helpers rather than raw secret files:

```bash
unset PYTHONPATH PYTHONHOME
<private-term>/.hermes/hermes-agent/venv/bin/python - <<'PY'
from hermes_cli.config import get_config_path, require_readable_config_before_write, fast_safe_load
from utils import atomic_yaml_write
path = get_config_path()
require_readable_config_before_write(path)
with open(path, encoding='utf-8') as f:
    cfg = fast_safe_load(f) or {}
server = cfg.setdefault('mcp_servers', {}).setdefault('openrouter', {})
server.update({'url': 'https://mcp.openrouter.ai/mcp', 'enabled': True, 'auth': 'oauth'})
tools = server.setdefault('tools', {})
tools.pop('include', None)
tools['exclude'] = ['chat-send']
...
Validate in a fresh process using the Hermes venv Python, because system `python3` may not have the optional `mcp` SDK installed:
...
assert 'mcp_openrouter_chat_send' not in names, 'chat-send is still exposed'
assert 'mcp_openrouter_models_list' in names, 'models-list missing'
print('\n'.join(n for n in names if n.startswith('mcp_openrouter_')))
PY
```
...
Before launching a batch, run this non-billable process-health preflight once in the same environment:
...
```bash
tmux new-session -d -s or-job-1 -x 120 -y 40 \
  "hermes chat --safe-mode --source tool --provider openrouter --model '<verified-model-slug>'"

tmux send-keys -t or-job-1 '<self-contained task brief>' Enter
```
