...
- For converting a read-only Daytona/Tailscale/VNC audit into an exact approval-gated execution manifest—with CLI/schema unit reconciliation, conservative spend guards, concrete-host grants/tests, browser-authenticated ephemeral joins, private-preview distinctions, objective probes, and verified teardown—load `references/daytona-tailnet-vnc-execution-manifests.md`.
