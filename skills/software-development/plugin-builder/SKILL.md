...
For deployments with separate plugin workshops, apply the role-based routing convention in `references/plugin-profile-routing.md` before choosing a target plugin path: user-specific/private plugins belong to the configured private workshop, while reusable/public plugins belong to the configured public plugin source workshop and remain identity-neutral.
...
When generated plugins are separated by privacy class, `$HERMES_HOME` should be the configured owning workshop profile described in `references/plugin-profile-routing.md`: the private workshop for user-specific/private plugins and the public plugin source workshop for reusable/public plugins. Treat writes to any other profile's `plugins/` tree as clarification-required unless the user explicitly approved that target.
...
Use a fresh-process real Hermes `PluginManager`/`PluginContext` discovery probe for registration feedback. Split discovery from handler execution: first load the package with **no tool or command calls** and prove enablement, declaration/registration parity, host dispatch, active-profile derivation, command conflicts, toolset filtering, and `register_skill(Path-to-SKILL.md)`. Before invoking any handler, classify it as local/pure, local-writing, networked, paid, destructive, or otherwise side-effecting. Invoke only an explicit allowlist of reviewed local/pure handlers by default; use missing-prerequisite rejection for networked handlers unless the user approved a real read-only sandbox, credentials, network access, and any spend. Never use a broad “call every registered handler” probe: an apparently harmless status, catalog, font, media, or metadata tool may contact a live service. Verify normal JSON, malformed-input rejection, and nonempty command output only for the approved allowlist, then confirm `hermes -p <profile> tools list` exposes the plugin toolset. If the real manager or approved service boundary cannot run, report a blocker instead of substituting a fake registration context or silently widening the probe.
...
Plugin name: ...
Owning plugin profile role: configured private workshop | configured public plugin source workshop | other explicitly approved target
Privacy/routing class: user-specific/private | reusable/public
Extension surface: ...
Goal: ...
...
- For browser/media/creative API plugin contracts, capability gating, paid-call approval, and real-service probes, load `references/browser-media-creative-api-plugins.md`.
- For lessons from validating an external Kanban runtime through real manager and service boundaries, load `references/external-kanban-runtime-validation-lessons.md`.
