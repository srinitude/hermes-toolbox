...
description: "Use when authoring or editing in-repo Hermes skills."
version: 1.2.0
...
2. **In-repo (this skill is about this case):** `<hermes-source>/skills/<category>/<name>/SKILL.md` — committed, shipped with the package. Use `write_file` + `git add`. `skill_manage(action='create')` does NOT target this tree.

## When to Use

- User asks you to add a skill "in this branch / repo / commit"
- You're committing a reusable workflow that should ship with hermes-agent
- You're editing an existing skill under `<hermes-source>/skills/` (use `patch` for small edits, `write_file` for rewrites; `skill_manage` still works for patch on in-repo skills, but not for `create`)
...
- `name` field present.
- `description` field present. The validator allows ≤ **1024 chars** (`MAX_DESCRIPTION_LENGTH`); authored skills require **< 60 chars** and one exact trigger class.
...
description: "Use when authoring or editing in-repo Hermes skills." # <60 chars
version: 1.1.0
author: Hermes Agent
license: MIT
metadata:
...
- Authored description: < 60 chars; platform validator: ≤ 1024 chars.
- Body after frontmatter: < 200 lines; full SKILL.md: ≤ 100,000 chars (enforced as `MAX_SKILL_CONTENT_CHARS`, ~36k tokens).
- Peer skills in `software-development/` sit at **8-14k chars**. Aim for that range. Split into `references/*.md` before the body reaches 200 lines or the file passes 20k chars.
...
2. **Choose the right context load.** A model-invoked Hermes skill pays for its description every turn. Keep it under 60 characters and limited to the exact trigger class so unrelated tasks don't load the skill. Put details in the body or linked references.
...
3. **Description too broad or long.** Keep it under 60 chars. Start with "Use when ..." and name the exact trigger class, not one task or generic behavior. "Use when debugging X" > "Debug X".
...
- [ ] Description is < 60 chars, starts with "Use when ...", and names only the exact trigger class
- [ ] Body after frontmatter is < 200 lines
- [ ] Total file ≤ 100,000 chars (aim for 8-15k)
- [ ] Structure: `# Title` → `## Overview` → `## When to Use` → body → `## Common Pitfalls` → `## Verification Checklist`
- [ ] Each ordered step has a checkable completion criterion
- [ ] Description excludes unrelated triggers and avoids duplicated body content
