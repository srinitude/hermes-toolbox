...
<!-- prompt-enhancer-honcho-propagation:start -->
## Prompt-Enhancer Honcho Memory Contract Propagation

The loaded `prompt-enhancer` must contain exactly one `honcho-persistent-memory-contract` marker pair. Apply that contract to every user reply in the builder loop and preserve it in every profile created through this skill.

1. Add relevant built-in/Honcho context, source conflicts, memory CRUD intent, store/target/peer, approval requirements, and read-back proof to the running profile spec. Do not enable or configure Honcho merely because the contract exists.
2. Distinguish standard shared-workspace/user-peer plus per-profile AI-peer behavior from explicitly approved strict workspace/user/AI isolation. Preserve the target profile’s approved peer model.
3. Treat prompt-enhancer as a required bootstrap artifact for blank, cloned, clone-all, clone-from, distribution-installed, and `--no-skills` creation paths handled by this skill. After creation, repair a missing/stale prompt-enhancer only from the validated canonical bootstrap source and only within the approved skill tree.
4. Copy only `SKILL.md` and allowlisted `references/`, `templates/`, `scripts/`, and `assets/`; preserve profile-specific extras and never copy memories, Honcho config, credentials, plugins, runtime state, or symlinks.
5. Before profile completion, prove the target prompt-enhancer has one byte-identical Honcho contract block, valid frontmatter/body/limits/local references, and fresh local plus enabled-only skill discovery. A missing tool, disabled skill, identity ambiguity, or unavailable Honcho provider is a blocker, not permission to weaken the contract.
6. Do not create, update, or delete real memory merely to test profile generation. Use a disposable `HERMES_HOME` and static/fresh-process validation.

Completion criterion: every profile created through `/profile-builder` can load the same validated memory-context/CRUD contract while retaining its own approved memory provider, Honcho peer isolation, and profile-local customizations.
<!-- prompt-enhancer-honcho-propagation:end -->
...
Every profile/config distribution created through this skill should carry the repository owner's approved authorship preferences unless the user explicitly overrides them in the final approval spec:

- Git author identity: `<repo-author-name> <repo-author-email>`.
- Commit message policy: never add `Co-authored-by`, AI co-authoring trailers, or AI-attribution lines.
- New repository-authored skills: `version: 0.1.0`, `author: <repo-author-name>`, and `license: Apache-2.0`.
...
After the profile exists, perform the approved copy with exact file tools or a vetted profile-distribution installer. Resolve the source and target homes first, copy only the allowlisted skill paths, compare the resulting tree hashes, and roll back the target skill tree if any validation fails. Do not paste a generic recursive copy script into an unverified environment.
...
- Source plugin role: the deployment-configured public plugin source profile's `plugins/kanban-workflow-control-plane/` package.
...
Do not install or enable this plugin by default in the configured private plugin workshop, the configured public plugin source workshop, or any profile matching `kanban-workflow-*`.

The public plugin source workshop is a source package location, not a consumer installation.
...
configuration:
  ...
  manifest_entry_shape:
    ...
    protected_input: not_required
    protected_input_handling: "none | env_or_auth_flow | user_action_required"
  ...
  profile_builder_bootstrap_plugins:
    required:
      - name: kanban-workflow-control-plane
        source_preference: configured_public_plugin_source_profile
        ...
        exclusions: [private_plugin_workshop, public_plugin_source_workshop, kanban-workflow-*]
...
2. **Use safe roots only.** Consider:
   ...
   - the plugins directory resolved for an explicitly approved source profile, using official profile/config path discovery rather than a constructed path,
...
search_files("plugin.y*ml", target="files", path="$HERMES_HOME/plugins")
read_file("$HERMES_HOME/plugins/example-plugin/plugin.yaml")
```
...
- Repository/authorship governance when a config repo or commits are involved:
  - Git author: `<repo-author-name> <repo-author-email>` unless explicitly overridden
  - Commit messages: no AI co-authoring or `Co-authored-by` trailers
  - New repository-authored skills: `version: 0.1.0`, `author: <repo-author-name>`, `license: Apache-2.0`
...
```bash
hermes -p example-profile config set agent.reasoning_effort medium
```
...
For independent fleet-wide compatibility, propagation, or completed-rollout acceptance reviews, load `references/fleet-compatibility-acceptance-audit.md`. It adds stable live discovery, exact canonical-block comparison, customization reconstruction, nested-symlink checks, fresh local/enabled-only discovery, disposable future-profile evidence, and adversarial verifier coverage without authorizing repairs.
...
- The selected profile's `config.yaml` through `hermes config` where possible
- The selected profile's `.env` through setup/auth/config flows for secrets only
- The selected profile's `SOUL.md`
- The selected profile's `skills/` tree through `skill_manage` or `hermes skills`
- The selected profile's `cron/` tree through `cronjob` or `hermes cron`
- The selected profile's `plugins/` tree only when explicitly approved in the final profile spec
...
- [ ] `hermes -p <profile-name> skills list` or direct `SKILL.md` validation proves all four bootstrap skills are available in the new profile, or blockers are reported honestly.
