...
- If the user supplies a raw API key in chat, do not repeat it in the profile spec; refer to `secret_ref:<KEY_NAME>` and the profile-local `.env`/secret-manager storage path.
